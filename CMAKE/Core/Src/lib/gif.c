...
u8 gif_check_head(FIL *file) {
    ...
    if (res) return 1;
    if ((gifversion[0] != 'G') || (gifversion[1] != 'I') ||
        (gifversion[2] != 'F') || (gifversion[3] != '8') ||
        ((gifversion[4] != '7') && (gifversion[4] != '9')) ||
        (gifversion[5] != 'a'))
        return 2;
    else
        return 0;
...
// ReadExtension
// Purpose:
// Reads an extension block. One extension block can consist of several data
// blocks. If an unknown extension block occures, the routine failes.
// 返回值:0,成功;
//  		 其他,失败
...
int gif_getnextcode(FIL *gfile, gif89a *gif) {
    ...
    if (i == j)
        ...
        Result =
            (long)gif->lzw->aBuffer[i] | ((long)gif->lzw->aBuffer[i + 1] << 8);
    else
        Result = (long)gif->lzw->aBuffer[i] |
                 ((long)gif->lzw->aBuffer[i + 1] << 8) |
                 ((long)gif->lzw->aBuffer[i + 2] << 16);
    Result =
        (Result >> (gif->lzw->CurBit & 0x7)) & _aMaskTbl[gif->lzw->CodeSize];
...
int gif_getnextbyte(FIL *gfile, gif89a *gif) {
    ...
    while ((Code = gif_getnextcode(gfile, gif)) >= 0) {
        if (Code == gif->lzw->ClearCode) {
            ...
            // Calculate the'special codes' independence of the initial code
            // size and initialize the stack pointer
            gif->lzw->CodeSize = gif->lzw->SetCodeSize + 1;
            gif->lzw->MaxCodeSize = gif->lzw->ClearCode << 1;
            gif->lzw->MaxCode = gif->lzw->ClearCode + 2;
            gif->lzw->sp = gif->lzw->aDecompBuffer;
            // Read the first code from the stack after clear ingand
            // initializing*/
            do {
        ...
        *(gif->lzw->sp)++ = gif->lzw->FirstCode = gif->lzw->aPrefix[Code];
        if ((Code = gif->lzw->MaxCode) < (1 << MAX_NUM_LWZ_BITS)) {
            ...
            ++gif->lzw->MaxCode;
            if ((gif->lzw->MaxCode >= gif->lzw->MaxCodeSize) &&
                (gif->lzw->MaxCodeSize < (1 << MAX_NUM_LWZ_BITS))) {
...
// DispGIFImage
// Purpose:
//    This routine draws a GIF image from the current pointer which should point
//    to a valid GIF data block. The size of the desired image is given in the
//    image descriptor.
// Return value:
//   0 if succeed
//   1 if not succeed
// Parameters:
//   pDescriptor  - Points to a IMAGE_DESCRIPTOR structure, which contains infos
//   about size, colors and interlacing. x0, y0       - Obvious. Transparency -
//   Color index which should be treated as transparent. Disposal     - Contains
//   the disposal method of the previous image. If Disposal == 2, the
//   transparent pixels
...
u8 gif_dispimage(FIL *gfile, gif89a *gif, u16 x0, u16 y0, int Transparency,
    ...
    gif_initlzw(gif, lzwlen);  // Initialize the LZW stack with the LZW code
                               // size
            ...
            if ((Index == OldIndex) && (XPos <= XEnd))
                ...
                if (Cnt) {
                    if (OldIndex != Transparency) {
                        pic_phy.draw_hline(XPos - Cnt - 1, YPos, Cnt + 1,
                                           *(pTrans + OldIndex));
                    } else if (Disposal == 2) {
                        pic_phy.draw_hline(XPos - Cnt - 1, YPos, Cnt + 1,
                                           bkcolor);
                    ...
                    if (OldIndex >= 0) {
                        if (OldIndex != Transparency)
                            pic_phy.draw_point(XPos - 1, YPos,
                                               *(pTrans + OldIndex));
                        else if (Disposal == 2)
    ...
    do {
        ...
        switch (Introducer) {
            case GIF_INTRO_IMAGE:  // 图像描述
                ...
                res = f_read(gfile, (u8 *)&gif->gifISD, 9,
                             (UINT *)&readed);  // 读取一个字节
                if (res) return 1;
                if (gif->gifISD.flag & 0x80)  // 存在局部颜色表
                {
                    ...
                    numcolors =
                        2 << (gif->gifISD.flag & 0X07);  // 得到局部颜色表大小
            ...
            case GIF_INTRO_EXTENSION:
                ...
                res = gif_readextension(gfile, gif, &TransIndex,
                                        &Disposal);  // 读取图像扩展块消息
                if (res) return 1;
                break;
            default:
...
// 退出当前解码.
void gif_quit(void) { gifdecoding = 0; }
...
u8 gif_decode(const u8 *filename, u16 x, u16 y, u16 width, u16 height) {
    ...
    if (res == 0)  // OK
    {
        ...
        if (res == 0)  // 打开文件ok
        {
            ...
            if (mygif89a->gifLSD.width > width ||
                mygif89a->gifLSD.height > height)
